...
public class SteeringArrive : MonoBehaviour {
	...
	public void Steer(Vector3 target)
	{
        ...
        {
            Vector3 desired_a = (target - move.transform.position).normalized * move.max_mov_velocity;

            if (distance <= slow_distance)
            {
                float factor = (move.transform.position - target).magnitude / slow_distance;
                desired_a *= factor;
            }

            Vector3 acceleration = desired_a - move.movement;
            acceleration = acceleration / time_to_target;
